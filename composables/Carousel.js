// sample carousel instance
//  const carousel = new Carousel(my-carousel, [yourImageArrayHere])
...
class Carousel {
  ...
  init() {
    ...
    this.dom.leftControl = this.dom.carousel.querySelector(
      ".carousel__controls--left"
    );
    this.dom.rightControl = this.dom.carousel.querySelector(
      ".carousel__controls--right"
    );
    if (this.settings.indicatorBar !== "off") {
      this.dom.indicatorBar =
    ...
    this.dom.viewportWrapper = this.dom.carousel.querySelector(
      ".carousel__viewport-wrapper"
    );

    // bind controller events
    this.dom.leftControl.addEventListener(
      "mousedown",
      this.backward.bind(this)
    );
    this.dom.rightControl.addEventListener(
      "mousedown",
      this.forward.bind(this)
    );
    this.dom.carousel.addEventListener(
      "keydown",
      this.keyDownControls.bind(this)
    );

    // bind touch events
    this.dom.viewportWrapper.addEventListener(
      "touchstart",
      this.rafManager,
      false
    );
  ...
  createCarouselElements() {
    const carouselElements = `
        <div class="carousel__viewport-wrapper">
          ...
          ${
            this.settings.indicatorBar === "exterior"
              ? '<div class="carousel__indicator-bar carousel__indicator-bar--exterior"></div>'
              : ""
          }
      `;
  ...
  buildCarousel() {
    ...
    if (this.dom.indicatorBar) {
      ...
      if (
        this.dom.indicatorBar.contains(
          document.querySelector(".carousel__indicator-wrapper")
        )
      ) {
        this.dom.indicatorBar.removeChild(
          document.querySelector(".carousel__indicator-wrapper")
        );
      }
  ...
  buildSlides() {
    ...
    this.dom.viewport.insertAdjacentHTML("beforeend", imageReel);

    this.dom.slideContainers = [
      ...this.dom.carousel.querySelectorAll(".carousel__slide-container"),
    ];
  ...
  addOrientationStyles(slide) {
    const imgAspectRatioQuotient = (
      slide.img.naturalWidth / slide.img.naturalHeight
    ).toFixed(2);
    ...
    if (this.settings.imageSize === "contain") {
      if (imgAspectRatioQuotient > viewAspectRatioQuotient) {
        ...
        const wrapperHeight =
          (viewAspectRatioQuotient / imgAspectRatioQuotient) * 100;
        ...
        const wrapperWidth =
          (imgAspectRatioQuotient / viewAspectRatioQuotient) * 100;
        slide.wrapper.style.width = `${wrapperWidth}%`;
      } else {
        slide.wrapper.classList.add(
  ...
  buildindicatorBar() {
    ...
    this.dom.indicatorBar.insertAdjacentHTML("beforeend", indicatorWrapper);

    document
      .querySelector(".carousel__indicator-wrapper")
      .addEventListener("click", this.jumpToMarker.bind(this));
    this.dom.indicatorMarkers = [
      ...document.querySelectorAll(".carousel__indicator-marker"),
    ];
  }

  updateindicatorBar() {
    if (this.state.currentMarker) {
      this.state.currentMarker.classList.remove(
        "carousel__indicator-marker--active"
      );
    }
    this.state.currentMarker = this.dom.indicatorMarkers[
      this.state.slidesArr[1].id
    ];
    this.state.currentMarker.classList.add(
      "carousel__indicator-marker--active"
    );
  }
  ...
  initialRender() {
    ...
    if (this.state.quantity > 2) {
      for (let i = 2; i < this.state.quantity; i++) {
        this.dom.slideContainers[this.state.slidesArr[i].id].classList.add(
          "hide"
        );
        this.dom.slideContainers[this.state.slidesArr[i].id].classList.add(
          "queue"
  ...
  jumpToMarker(e) {
    ...
    if (e.target.classList.contains("carousel__indicator-marker")) {
      this.dom.slideContainers.forEach((el) =>
        el.classList.add("carousel__slide-container--jumper")
      );

      jumpAmount =
        this.dom.indicatorMarkers.indexOf(e.target) -
        this.dom.indicatorMarkers.indexOf(this.state.currentMarker);
        ...
        jumpInterval = setInterval(
          function () {
            if (jumpTicker === jumpAmount) {
              ...
              this.dom.slideContainers.forEach((el) =>
                el.classList.remove("carousel__slide-container--jumper")
              );
              return;
            }
            this.forward(e);
  ...
  touchStartRenders(e) {
    ...
    if (
      this.settings.transition === "viewmaster" ||
      this.settings.transition === "horizontal"
    ) {
      this.touchCtrls.slideOffset.left =
        this.viewport.left - this.viewport.width;
      this.touchCtrls.slideOffset.right =
        this.viewport.left + this.viewport.width;
    }
    if (this.settings.transition === "vertical") {
      this.touchCtrls.slideOffset.top =
        this.viewport.top - this.viewport.height;
      this.touchCtrls.slideOffset.bottom =
        this.viewport.top + this.viewport.height;
    }
    if (this.settings.transition === "viewmaster") {
      this.touchCtrls.viewmasterFactor =
        this.viewport.height / 2 / this.viewport.width;
    }
  ...
  touchMoveRenders(e) {
    ...
    const touchobj = e.touches[0];
    if (
      document.elementFromPoint(touchobj.clientX, touchobj.clientY) !== e.target
    ) {
    ...
    switch (this.settings.transition) {
      case "viewmaster":
        if (touch.distX > 0) {
          touch.slides[1].style.transform = `translate(${touch.distX}px, ${
            touch.distX * touch.viewmasterFactor
          }px)`;
        } else {
          touch.slides[1].style.transform = `translate(${touch.distX}px, ${
            touch.distX * -touch.viewmasterFactor
        ...
        if (touch.slideOffset.left + touch.distX >= touch.slideOffset.left) {
          touch.slides[0].style.transform = `translate(${
            touch.distX - viewportWidth
          }px, ${
            viewportHeight * 0.5 - touch.distX * touch.viewmasterFactor
          }px)`;
        } else {
          touch.slides[0].style.transform = null;
        }
        if (touch.slideOffset.right + touch.distX <= touch.slideOffset.right) {
          touch.slides[2].style.transform = `translate(${
            viewportWidth + touch.distX
          }px, ${
            viewportHeight * 0.5 + touch.distX * touch.viewmasterFactor
          }px)`;
        } else {
      ...
      case "vertical":
        ...
        if (touch.slideOffset.top + touch.distY >= touch.slideOffset.top) {
          touch.slides[0].style.transform = `translate(0, ${
            touch.distY - viewportHeight
          }px)`;
        } else {
          touch.slides[0].style.transform = null;
        }
        if (
          touch.slideOffset.bottom + touch.distY <=
          touch.slideOffset.bottom
        ) {
          touch.slides[2].style.transform = `translate(0, ${
            viewportHeight + touch.distY
          }px)`;
        } else {
          touch.slides[2].style.transform = null;
        }
  ...
  touchEndRenders(e) {
    this.dom.viewportWrapper.removeEventListener(
      "touchmove",
      this.rafManager,
      false
    );
    ...
    const dist =
      this.settings.transition === "vertical"
  ...
  backwardRender() {
    if (this.state.quantity === 2) {
      this.dom.slideContainers[this.state.slidesArr[1].id].classList.remove(
        "queue"
      );
      this.dom.slideContainers[this.state.slidesArr[1].id].classList.add(
        "prev"
      );
      this.dom.slideContainers[this.state.slidesArr[0].id].classList.add(
        "queue"
      );
      ...
      this.dom.slideContainers[this.state.slidesArr[0].id].classList.remove(
        "queue"
      );
      this.dom.slideContainers[this.state.slidesArr[1].id].classList.remove(
        "hide"
      ...
      this.dom.slideContainers[this.state.slidesArr[2].id].classList.add(
        "queue"
      );
    }
  }
  ...
  forwardRender() {
    if (this.state.quantity === 2) {
      ...
      this.dom.slideContainers[
        this.state.slidesArr[this.state.quantity - 1].id
      ].classList.add("hide");
      this.dom.slideContainers[
        this.state.slidesArr[this.state.quantity - 1].id
      ].classList.add("queue");
  ...
  backward(e) {
    ...
    this.state.currentImage = this.dom.slideContainers[
      this.state.slidesArr[1].id
    ];
    this.boundSliderTransitionend = this.sliderTransitionend.bind(this);
    this.state.currentImage.addEventListener(
      "transitionend",
      this.boundSliderTransitionend
    );
  ...
  sliderTransitionend() {
    ...
    this.touchCtrls.slides.forEach((val) =>
      val.classList.remove("carousel__slide-container--flick-trans")
    );
    if (this.dom.indicatorBar) this.updateindicatorBar();
    this.state.currentImage.removeEventListener(
      "transitionend",
      this.boundSliderTransitionend
    );
  }
